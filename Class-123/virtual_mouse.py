...
def countFingers(image, hand_landmarks, handNo=0):
	...
	if hand_landmarks:
		...
		fingertipY=landmarks[8].y*height
		thumbX=landmarks[4].x*width
		thumbY=landmarks[4].y*height
		centerX=int((fingertipX+thumbX)/2)
		centerY=int((fingertipY+thumbY)/2)
		# Draw a LINE between FINGER TIP and THUMB TIP
		cv2.line(image,(fingertipX,fingertipY),(thumbX,thumbY),(0,100,0),3)

		# Draw a CIRCLE on CENTER of the LINE between FINGER TIP and THUMB TIP
		cv2.circle(image,(centerX,centerY),25,(200,0,200),3)

		# Calculate DISTANCE between FINGER TIP and THUMB TIP
		distance=math.sqrt((fingertipX-thumbX)**2+(fingertipY-thumbY)**2)

		# Set Mouse Position on the Screen Relative to the Output Window Size	
		relMouseX=(centerX/width)*screen_width
		relMouseY=(centerY/height)*screen_height
		mouse.position=(relMouseX,relMouseY)
